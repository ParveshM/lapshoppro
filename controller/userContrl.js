...
const {
  ...
  creditforNewUser,
} = require("../helpers/referralHelper");
...
const insertUser = async (req, res) => {
  try {
    ...
    if (checkData) {
      return res.render("./shop/pages/register", {
  ...
  try {
    ...
    if (enteredOTP == storedOTP) {
      ...
      if (!userFound && user.referralCode) {
        req.flash(
          "warning",
          "Registration success , Please login , Invalid referral code!"
        );
      } else {
  ...
  try {
    ...
    if (updatedName == "") {
      return res.json({
        ...
        message: "UserName is not updated",
      });
    }
...
const viewWalletHistory = asyncHandler(async (req, res) => {
  try {
    ...
    if (findWallet.wallet) {
      ...
      const walletTransaction = await Wallet.findById({
        _id: walletId,
      }).populate({
        path: "transactions",
        options: {
